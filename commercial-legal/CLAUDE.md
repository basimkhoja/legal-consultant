...
4. On first run, if a populated CLAUDE.md exists at either legacy location but not at the config path,
   ...
   (a) the upstream config path ~/.claude/plugins/config/claude-for-legal/commercial-legal/CLAUDE.md;
   (b) the upstream cache path ~/.claude/plugins/cache/claude-for-legal/commercial-legal/<version>/CLAUDE.md for any version.
...
**Never:** [PLACEHOLDER]
**Forum:** [PLACEHOLDER — courts | arbitration]
**Arbitration institution and seat:** [PLACEHOLDER — e.g. SCCA, Riyadh]
**Language of proceedings and prevailing contract language:** [PLACEHOLDER]
**Jurisdiction playbook defaults applied:** [PLACEHOLDER — rows accepted from `references/jurisdictions/<code>/playbook-defaults.md`]
...
**Value thresholds (in the profile currency, see `## Jurisdiction`):** [PLACEHOLDER]
