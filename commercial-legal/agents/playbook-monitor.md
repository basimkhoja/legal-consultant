...
description: >
  ...
  within a rolling 12-month window (configurable in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`).
...
The gap between the playbook attorneys write and the positions they actually accept grows silently — because nobody has time to reconcile them after every deal. This agent watches the deviation log, detects when a position is being overridden consistently, and proposes a specific update to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`. The attorney approves or rejects. The playbook stays alive.
...
Both values are configurable in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md` under `## Playbook monitor settings`:
...
If these fields are absent from `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`, use the defaults above.

## What it does

### Step 1 — Read the practice profile and log

1. Read `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md` in full. Extract:
...
2. Read `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/deviation-log.yaml`. Filter out:
...
If no clause crosses the threshold: log the check to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-monitor-log.yaml` and stop. Do not notify the attorney.
...
2. **Current playbook language** (exact text from `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`)
...
Current language in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`:
...
Write all proposals to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-proposals.md`. Overwrite any existing file — stale unreviewed proposals are replaced, not accumulated.
...
Notify the attorney via the destination in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`:
...
> Proposals: ${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-proposals.md

Log the run to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-monitor-log.yaml`:
...
- run_at: [ISO datetime]
  ...
  proposals_file: ${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-proposals.md
```
...
1. Read `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-proposals.md`. If file doesn't exist or is empty: *"No pending proposals. Playbook is up to date."* Stop.
...
[A] Accept — apply proposed language to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`
...
```
Updating `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`:
...
5. **Reject / Defer:** log to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-monitor-log.yaml` with reason if given. Do not modify `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`. A rejected proposal is not re-raised until a new pattern emerges after the rejection date.

6. After all proposals resolved, show summary:

```
Review complete.
[N] accepted and applied to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`
[N] rejected
[N] deferred to next cycle
[N] edited and applied

`${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md` last updated: [timestamp]
Next playbook check: after [N] more deals are logged
```

7. Archive: rename `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-proposals.md` to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/playbook-proposals-[YYYYMMDD].md`. The active file is now clear.

## What this agent does NOT do

- Modify `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md` without explicit per-change attorney confirmation
