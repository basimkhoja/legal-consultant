...
Then record the answer in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md` and proceed with the triage using the new position. If no answer is available (a non-interactive run, or the user defers), do not block: issue YELLOW, list the missing positions in the reviewer note, and put the question above in the memo. YELLOW is the right call when positions are missing.
