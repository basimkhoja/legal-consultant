# Legal Consultant — shared guardrails for Gemini CLI

> Generated by `scripts/build-runtimes.py` from the plugin `CLAUDE.md` templates; do not edit. Regenerate with `python3 scripts/build-runtimes.py`.

Legal Consultant is a jurisdiction-pluggable fork of Anthropic's `claude-for-legal`. Under Claude Code the shared guardrails live in each plugin's `CLAUDE.md`; Gemini CLI does not read those files, so they are reproduced here and apply to every `commercial-legal-*`, `corporate-legal-*`, and `employment-legal-*` skill.

## Runtime rules

- **Practice profile path.** `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/<plugin>/CLAUDE.md` for `commercial-legal`, `corporate-legal`, `employment-legal`, plus the shared `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/company-profile.md`. Any text that still says `~/.claude/plugins/config/claude-for-legal/<plugin>/…` means this path. If a populated profile exists only at the old path, copy it forward once, then use the new path.
- **Jurisdiction tree.** The `legal-consultant-jurisdictions` skill carries `references/jurisdictions/`. Skills resolve the profile's jurisdiction codes against `REGISTRY.md` and load `<code>/MANIFEST.md`, `INDEX.md`, and the instrument files. Unpopulated code: stop.
- **Primary-source research.** For a non-`usa` code use the portal named in the manifest, e.g. `python3 scripts/fetch-law.py --portal boe --id <guid> --lang ar` (the script is under the jurisdiction skill's `scripts/` folder when installed from this repo) or `curl` of the portal page; the built-in web-fetch tool rejects the laws.boe.gov.sa TLS chain. Tag `[BOE — Arabic]` / `[BOE — official English]`. On failure, apply the no-silent-supplement rule.
- **Not available in this runtime.** Claude Code subagents (`agents/*.md`), hooks, and scheduled agents have no direct equivalent here; the scheduled behaviours (renewal watcher, leave tracker, deal debrief, playbook monitor, data-room watcher) are run on demand by invoking the matching skill. See `docs/runtime-matrix.md`.
- **MCP servers.** Configure connectors in `gemini-extension.json` → `mcpServers` or your `~/.gemini/settings.json`. None are required for the jurisdiction files; the upstream legal-research connectors (CourtListener, Westlaw) are only relevant when `usa` is in the footprint.

## Skills

- `/commercial-legal:amendment-history` (skill `commercial-legal-amendment-history`) — Trace how a contract has changed across its base agreement and all amendments — either a summary of all changes over time, or a provision trace for a specific c
- `/commercial-legal:cold-start-interview` (skill `commercial-legal-cold-start-interview`) — Run the cold-start interview to learn your commercial contracts practice and write your team practice profile
- `/commercial-legal:customize` (skill `commercial-legal-customize`) — Guided customization of your commercial contracts practice profile — change one thing without re-running the whole cold-start interview
- `/commercial-legal:escalation-flagger` (skill `commercial-legal-escalation-flagger`) — Route a contract issue to the right approver per the escalation matrix in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`, and draft 
- `/commercial-legal:matter-workspace` (skill `commercial-legal-matter-workspace`) — Manage matter workspaces — new, list, switch, close, or detach (practice-level)
- `/commercial-legal:nda-review` (skill `commercial-legal-nda-review`) — Reference: fast triage of inbound NDAs into GREEN / YELLOW / RED so the team only spends lawyer time on the ones that need it
- `/commercial-legal:renewal-tracker` (skill `commercial-legal-renewal-tracker`) — Show contracts with cancel-by deadlines coming up and warn before notice windows close, working from a maintained renewal register
- `/commercial-legal:review` (skill `commercial-legal-review`) — Review a vendor agreement, NDA, or SaaS subscription against your playbook
- `/commercial-legal:review-proposals` (skill `commercial-legal-review-proposals`) — Review and approve (or reject) pending playbook update proposals from the playbook-monitor agent and apply approved changes to the practice profile
- `/commercial-legal:saas-msa-review` (skill `commercial-legal-saas-msa-review`) — Reference: review of SaaS subscription agreements with attention to the terms that matter most in subscription deals — auto-renewal mechanics, price escalation,
- `/commercial-legal:stakeholder-summary` (skill `commercial-legal-stakeholder-summary`) — Translates a contract review into a summary the business stakeholder will actually read
- `/commercial-legal:vendor-agreement-review` (skill `commercial-legal-vendor-agreement-review`) — Reference: review of an inbound vendor agreement against the team playbook in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/commercial-legal/CLAUDE.md`
- `/corporate-legal:ai-tool-handoff` (skill `corporate-legal-ai-tool-handoff`) — Detects when Luminance, Kira, or a similar bulk-review tool is in use, hands off the high-volume clause extraction to it, and QAs its output per the trust level
- `/corporate-legal:board-minutes` (skill `corporate-legal-board-minutes`) — Drafts board or committee meeting minutes in your house format
- `/corporate-legal:closing-checklist` (skill `corporate-legal-closing-checklist`) — What's blocking close — maintain the closing checklist with status, critical path, and days to close
- `/corporate-legal:cold-start-interview` (skill `corporate-legal-cold-start-interview`) — House cold-start interview (jurisdiction, request list + prior memo), or --new-deal for deal-specific context
- `/corporate-legal:customize` (skill `corporate-legal-customize`) — Guided customization of your corporate practice profile — change one thing without re-running the whole cold-start interview
- `/corporate-legal:deal-team-summary` (skill `corporate-legal-deal-team-summary`) — Aggregate diligence findings into a deal team briefing at the right altitude for the audience — exec summary for leadership, working summary for the team
- `/corporate-legal:diligence-issue-extraction` (skill `corporate-legal-diligence-issue-extraction`) — Read VDR documents and extract issues per house categories and materiality thresholds, producing findings in house memo format
- `/corporate-legal:entity-compliance` (skill `corporate-legal-entity-compliance`) — Entity compliance tracker — initialize, report upcoming deadlines, update status, run health audit, export to CSV
- `/corporate-legal:integration-management` (skill `corporate-legal-integration-management`) — Post-closing M&A integration tracker — phased workplan, consent tracking, contract assignment at scale, weekly status reports
- `/corporate-legal:material-contract-schedule` (skill `corporate-legal-material-contract-schedule`) — Build the material contracts disclosure schedule from diligence findings, applying the purchase agreement's Material Contract definition and formatting per the 
- `/corporate-legal:matter-workspace` (skill `corporate-legal-matter-workspace`) — Manage matter workspaces — create, list, switch, close, or detach the active matter so multi-client practitioners keep one client's context separate from every 
- `/corporate-legal:tabular-review` (skill `corporate-legal-tabular-review`) — Tabular review — one row per document, one column per data point, every cell cited to source
- `/corporate-legal:written-consent` (skill `corporate-legal-written-consent`) — Draft a written consent or resolution by circulation of the board or a committee in house format, with precedent search from the consents repository
- `/employment-legal:cold-start-interview` (skill `employment-legal-cold-start-interview`) — Cold-start setup — learns your jurisdictional footprint and escalation rules from your handbook and termination memos
- `/employment-legal:customize` (skill `employment-legal-customize`) — Guided customization of your employment practice profile — change one thing without re-running the whole cold-start interview
- `/employment-legal:expansion-kickoff` (skill `employment-legal-expansion-kickoff`) — Kick off international expansion planning for a new country — gathers intake, runs EOR vs
- `/employment-legal:expansion-update` (skill `employment-legal-expansion-update`) — Update the status of an in-progress international expansion project — recalculates what is now unblocked, flags anything overdue, and surfaces the next prioriti
- `/employment-legal:handbook-updates` (skill `employment-legal-handbook-updates`) — Diff a proposed handbook or work-regulations change against the current version, flag ripple effects, jurisdiction-supplement impacts and any regulatory re-appr
- `/employment-legal:hiring-review` (skill `employment-legal-hiring-review`) — Review an offer letter or employment contract and any restrictive covenants for the jurisdiction resolved from the practice profile — work-permit, nationalisati
- `/employment-legal:internal-investigation` (skill `employment-legal-internal-investigation`) — Reference: shared framework for managing internal investigations from intake through final memo — privileged investigation log, document processing with needle-
- `/employment-legal:international-expansion` (skill `employment-legal-international-expansion`) — Reference: implementation-planning framework for international hiring — EOR vs
- `/employment-legal:investigation-add` (skill `employment-legal-investigation-add`) — Add data to an open investigation — documents, interview notes, or observations
- `/employment-legal:investigation-memo` (skill `employment-legal-investigation-memo`) — Draft or update the privileged investigation memo from the investigation log
- `/employment-legal:investigation-open` (skill `employment-legal-investigation-open`) — Open a new internal investigation matter — runs intake, generates the sources checklist, and creates the persistent investigation log
- `/employment-legal:investigation-query` (skill `employment-legal-investigation-query`) — Ask questions against an open investigation log — what witnesses said, where accounts conflict, what gaps exist, what the strongest evidence is on each issue
- `/employment-legal:investigation-summary` (skill `employment-legal-investigation-summary`) — Draft an audience-specific summary from the privileged investigation memo — HR, leadership, or outside counsel versions
- `/employment-legal:leave-tracker` (skill `employment-legal-leave-tracker`) — Check open leaves for deadline alerts and required decisions
- `/employment-legal:log-leave` (skill `employment-legal-log-leave`) — Add a new leave to the leave register with the minimum information needed to start tracking the statutory clocks of the applicable jurisdiction (for `usa`: desi
- `/employment-legal:matter-workspace` (skill `employment-legal-matter-workspace`) — Manage matter workspaces — new, list, switch, close, or detach (practice- level)
- `/employment-legal:policy-drafting` (skill `employment-legal-policy-drafting`) — Draft an employment policy or work-regulations section with jurisdiction supplements where the law differs across the footprint (by state for `usa`; by national
- `/employment-legal:termination-review` (skill `employment-legal-termination-review`) — Termination review — high-risk flag detection, end-of-service award and compensation computation, notice, final settlement, and release, for the jurisdiction re
- `/employment-legal:wage-hour-qa` (skill `employment-legal-wage-hour-qa`) — Jurisdiction-aware wage/hour and employment Q&A — working hours, overtime, rest and leave, wage payment, deductions, final settlement, limitation — answered for
- `/employment-legal:worker-classification` (skill `employment-legal-worker-classification`) — Classify a proposed worker engagement — employee, contractor, seconded, agency or outsourced — by running the tests and categories of the jurisdiction resolved 
