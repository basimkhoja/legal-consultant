...
4. On first run, if a populated CLAUDE.md exists at either legacy location but not at the config path,
   ...
   (a) the upstream config path ~/.claude/plugins/config/claude-for-legal/corporate-legal/CLAUDE.md;
   (b) the upstream cache path ~/.claude/plugins/cache/claude-for-legal/corporate-legal/<version>/CLAUDE.md for any version.
...
|---|---|---|
| VDR (Intralinks, Datasite, Box) | [✓ / ✗] | Diligence pulls from local folder; user drops docs in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/corporate-legal/deals/[code]/vdr-mirror/` |
