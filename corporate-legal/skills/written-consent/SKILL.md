...
If (a) no consents repository is configured in `## Board & Secretary` → Consents repository, AND (b) no seed consent document has been uploaded this session AND the `## Board & Secretary` → Consent format section is empty or still placeholder (a filled Consent format section, with resolution language, recital depth and signature rules, satisfies (b) even if it names no seed document), **STOP before drafting**. Do not proceed to Step 1 intake, do not draft from a generic template, do not "get started" with a filler format.
...
for a JSC board; Art. 165 register for an LLC].
...
- **Partners of an LLC** — Art. 166: written vote circulated to the partners; > 50% of the capital, with the second-round meeting rule if the majority is not reached; Art. 172: ≥ 75% of the capital for amendments to the articles, and the items that need unanimity; Art. 165: entered in the minutes register; Regs Art. 63: manager removal at > 50%.
...
- **Conflicted directors and managers** — JSC board: Arts. 71-72 (disclosure and abstention); LLC managers: Art. 27 (partners' authorisation of a manager's interest) and Art. 164 (no vote on his own removal); CGR Art. 42 for a listed JSC.
...
Notice to non-signatories, in these rows, is the circulation-to-all requirement (Art. 82, Arts. 100-101, Art. 166): the resolution must reach every member or partner. Under Art. 82 and Arts. 100-101 any member's or shareholder's written request for a meeting defeats the circulation; under Art. 166 the row gives no such veto, but if the majority is not reached on the first consultation a meeting must be called. No separate post-signature notice period is in the files; if the user asks for one, tag `[no rule in ksa files — verify]`.
