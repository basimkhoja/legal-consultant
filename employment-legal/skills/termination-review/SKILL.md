...
4. **Emit the number with the computed-number tag and the inputs**, e.g.
   ...
   their own tags; do not fold them into the award. On a run where the
   portal was not read, the number carries only the computed-number tag
   with the file's settled date; never add `[BOE — Arabic]` or
   `[BOE — official English]` to a computed number.
