...
4. On first run, if a populated CLAUDE.md exists at either legacy location but not at the config path,
   copy it forward once to the config path before proceeding. Legacy locations, checked in order:
   (a) the upstream config path ~/.claude/plugins/config/claude-for-legal/employment-legal/CLAUDE.md;
   (b) the upstream cache path ~/.claude/plugins/cache/claude-for-legal/employment-legal/<version>/CLAUDE.md for any version.
   The config root is ${LEGAL_CONSULTANT_HOME:-~/.legal-consultant} — the environment variable relocates it; the
   default is ~/.legal-consultant. The same path is used by every runtime (Claude Code, Codex CLI, Gemini CLI).
5. This file (the one you are reading) is the TEMPLATE. It ships with the plugin and shows the
   structure the config should have. It is replaced on every plugin update. Never write user data here.

**Shared company profile.** Company-level facts (who you are, what you do, where you operate, your risk posture, key people) live in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/company-profile.md` — one level above this file, shared by all 12 plugins. Read it before this plugin's practice profile. If it doesn't exist, this plugin's setup will create it.
-->
...
|---|---|---|
| HRIS (Workday, BambooHR, Rippling, ADP) | [✓ / ✗] | Leave data tracked in `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/employment-legal/leave-register.yaml`; manual entry via `/employment-legal:log-leave` |
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/employment-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/employment-legal/CLAUDE.md` (and `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
**Primary jurisdiction and sub-jurisdictions with employees:** [PLACEHOLDER — codes; for `usa` list the states, for `che` the cantons, for `gbr` the nations]
**Other countries with employees:** [PLACEHOLDER — codes]
**Headcount by nationality:** [PLACEHOLDER — nationals / non-nationals, where a nationalisation quota applies]
**Nationalisation-quota band:** [PLACEHOLDER — e.g. Nitaqat band per establishment and activity, or N/A]
**Work-permit sponsorship:** [PLACEHOLDER — do we sponsor; pipeline; restricted professions]
**Contract types in use:** [PLACEHOLDER — fixed-term / indefinite / part-time / remote; documentation platform]
**Social-insurance and labour-platform registrations:** [PLACEHOLDER — e.g. GOSI number, Qiwa establishment, Mudad wage files]
...
**Statutory end-of-service entitlement:** [PLACEHOLDER — from `references/jurisdictions/<code>/labor-law.md`; the skill computes it]
**Standard severance above the statutory entitlement:** [PLACEHOLDER — formula or none]
**Standard notice periods by contract type:** [PLACEHOLDER — from the jurisdiction file unless the contract gives more]
...
**Jurisdiction supplements:** [PLACEHOLDER — which jurisdictions, sub-jurisdictions, or segments (nationality, sector, gender-specific rules) have addenda]
**Regulatory approval of work regulations:** [PLACEHOLDER — e.g. HRSD-certified work regulations, date; or N/A]
**Language versions:** [PLACEHOLDER — which prevails]
...
**Leave data access:** [Legal has read access / manual — `${LEGAL_CONSULTANT_HOME:-~/.legal-consultant}/employment-legal/leave-register.yaml`]
...
| Agency complaint or claim (the labour authority, labour court, or equivalent named in `references/jurisdictions/<code>/labor-dispute-route.md`) | — | [GC immediately] | Always |
